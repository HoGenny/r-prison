...
class TodoService:
    ...
    async def get_todo(
        self, session: AsyncSession, user_id: int, todo_id: int
    ) -> Todo:  # todo 1건 상세 검색
    ...
    async def create_todo(
        self, session: AsyncSession, user_id: int, data: TodoCreate
    ) -> Todo:  # todo 생성
    ...
    async def update_todo(
        self, session: AsyncSession, user_id: int, todo_id: int, data: TodoUpdate
    ) -> Todo:  # todo 수정
    ...
    async def complete_todo(
        ...
        async with session.begin():
            ...
            if stat is None:
                yesterday = todo.scheduled_for - timedelta(
                    days=1
                )  # 연속도전 계산을 위한 어제 연속일 계산
    ...
    async def uncomplete_todo(self, session: AsyncSession, user_id: int, todo_id: int) -> None:
        ...
        async with session.begin():
            ...
            if user.rp_balance < todo.reward_rp:
                raise AppError(
                    "RP 잔액이 부족하여 완료 취소할 수 없음",
                    status_code=400,
                    code="insufficient_rp",
                )
            ...
            if stat is None:
                ...
                raise AppError(
                    "DailyStat not found for uncomplete", status_code=400, code="stat_missing"
                )
